...
public class TokenProviderTest {
    ...
    void generateToken() {
        ...
        //토큰을 만들 때 클레임으로 넣어둔 id값이 given절에서 만든 유저ID와 동일한지 확인한다.
    ...
    void validToken_invalidToken() {
        //given
        //jjwt라이브러리를 사용해 토큰을 생성한다.
        //이때 만료 시간은 1970년 1월 1일부터 현재 시간을 밀리초 단위로 치환한 값(new Date().getTime())에 1000을 빼, 이미 만료된 토큰으로 생성한다.
    ...
    void validToken_validToken() {
        //given
        //jjwt라이브러리를 사용해 토큰을 생성한다.
        //만료 시간은 현재 시간으로부터 14일 뒤로, 만료되지 않은 토큰으로 생성한다.
    ...
    void getAuthentication() {
        //given
        //jjwt라이브러리를 사용해 토큰을 생성한다.
        //이때 토큰의 제목인 subject는 "user@email.com"라는 값을 사용한다.
        ...
        //when
        //토큰제공자의 getAuthentication()메서드를 호출해 인증 객체를 반환받는다.
    ...
    void getUserId(){
        //given
        //jjwt라이브러리를 사용해 토큰을 생성한다.
        //이때 클레임을 추가한다. 키는 "id",값은 1이라는 유저 ID임
        Long userId=1L;
        String token=JwtFactory.builder()
        ...
        //then
        //반환받는 유저ID가 given절에서 설정한 유저Id값인 1과 같은지 확인한다.
